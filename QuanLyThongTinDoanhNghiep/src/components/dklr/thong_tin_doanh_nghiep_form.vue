      ...
      >
        <v-layout row wrap class="ml-0">
          ...
          <v-flex xs12 sm12 v-if="thongTinDoanhNghiep['applicantStatus'] === 3 || thongTinDoanhNghiep['applicantStatus'] === '3'">
            <label>Ghi chú: </label> <label class="bold">{{thongTinDoanhNghiep['applicantRemarks']}}</label>
          </v-flex>

          <v-flex xs12 sm12 class="pt-2" v-if="thongTinDoanhNghiep['applicantProfileCOP']">
            <label>Mã CSSX: <span style="color: red;">(*)</span></label></label>
            <v-text-field
              :disabled="true"
              outline
              v-model="thongTinDoanhNghiep['applicantProfileCOP']"
              placeholder="Nhập mã doanh nghiệp"
              clearable
              class="py-0"
              required
              :rules="[v => !!v || 'Trường yêu cầu bắt buộc']"
              :class="{'track-change': isChange('applicantProfileCOP')}"
            ></v-text-field>
          </v-flex>

          <v-flex xs12 sm12 class="pt-2">
            <label>Tên doanh nghiệp: <span style="color: red;">(*)</span></label></label>
            <v-text-field
          ...
          <v-flex xs12 sm12>
            <label>Địa chỉ trụ sở chính (Theo Giấy chứng nhận đăng ký doanh nghiệp): <span style="color: red;">(*)</span></label></label>
            <v-text-field
              :disabled="disabledForm"
              outline
          ...
          <v-flex xs12 sm4>
            <label>Mã số doanh nghiệp (mã số thuế):</label>
            <v-text-field
              :disabled="true"
              outline
          ...
          <v-flex xs12 sm4>
            ...
            <v-text-field
              :disabled="disabledForm"
              outline
              v-model="thongTinDoanhNghiep['applicantissueplace']"
              placeholder="Nhập nơi cấp"
              clearable
              class="py-0 pr-1"
              required
              :rules="[v => !!v || 'Trường yêu cầu bắt buộc']"
              :class="{'track-change': isChange('applicantissueplace')}"
            ></v-text-field>
          </v-flex>
            ...
            <v-menu
              ...
              <template>
                <v-text-field
                  slot="activator"
                  class="py-0 pr-1"
                  v-model="thongTinDoanhNghiep['applicantissuedate']"
                  outline
                  placeholder="Nhập ngày cấp"
                  @blur="ngayCap = parseDate(thongTinDoanhNghiep['applicantissuedate'])"
                  required
                  :rules="[v => !!v || 'Trường yêu cầu bắt buộc']"
          ...
          <v-flex xs12 sm12>
            <label  v-if="(thongTinDoanhNghiep['applicantIdentityType'] === 2 || thongTinDoanhNghiep['applicantIdentityType'] === '2')">
              Giấy chứng nhận đăng ký doanh nghiệp:
            </label>
            <label  v-else>Giấy chứng nhận đăng ký kinh doanh:</label>
          </v-flex>
          <v-flex xs12 sm8>
            <v-layout row wrap class="ml-0">
              ...
              <v-flex xs12 sm3 class="text-xs-center">
                <v-chip label v-if="!disabledForm" style="cursor: pointer;" outline color="blue" @click="xacNhanTaiFileGCNDangKy()">Xác nhận tải file</v-chip>
                <v-chip :disabled="!thongTinDoanhNghiep['applicantIdentityFileEntryId'] || thongTinDoanhNghiep['applicantIdentityFileEntryId'] === '0'" v-else label style="cursor: pointer; pointer-events: auto;" outline color="blue" @click="xemGCNDangKy()">
                  <v-icon
                    size="17"
                    style="font-size: 15px"
                    :color="thongTinDoanhNghiep['applicantIdentityFileEntryId'] && thongTinDoanhNghiep['applicantIdentityFileEntryId'] !== '0' ? 'green' : 'red'
                    "
                    >
                    {{
                      thongTinDoanhNghiep['applicantIdentityFileEntryId'] && thongTinDoanhNghiep['applicantIdentityFileEntryId'] !== '0' ? "done" : "cancel"
                    }}
                  </v-icon>
                  Xem GCN
                </v-chip>
              ...
              <v-flex xs12 sm4 class="pt-2">
                <input v-if="!disabledForm" type="file" name="fileGCN" id="fileGCN">
              </v-flex>
            </v-layout>
          </v-flex>
          <v-flex xs12 sm4>
          ...
          <v-flex xs12 sm12>
            <label>Giấy chứng nhận đủ điều kiện sản xuất, lắp ráp ô tô (Với loại hình sản xuất Trên cơ sở tổng thành rời):</label>
          </v-flex>
          <v-flex xs12 sm8>
            <v-layout row wrap class="ml-0">
              <v-flex xs12 sm2 class="pt-2">
              ...
              <v-flex xs12 sm3 class="text-xs-center">
                <v-chip v-if="!disabledForm" label style="cursor: pointer;" outline color="blue" @click="xacNhanTaiFileĐuieuKien()">Xác nhận tải file</v-chip>

                <v-chip :disabled="!thongTinDoanhNghiep['applicantXcgFileEntryId'] || thongTinDoanhNghiep['applicantXcgFileEntryId'] === '0'" v-else label style="cursor: pointer; pointer-events: auto;" outline color="blue" @click="xemGCNĐuieuKien()">
                  <v-icon
                    size="17"
                    style="font-size: 15px"
                    :color="thongTinDoanhNghiep['applicantXcgFileEntryId'] && thongTinDoanhNghiep['applicantXcgFileEntryId'] !== '0' ? 'green' : 'red'
                    "
                    >
                    {{
                      thongTinDoanhNghiep['applicantXcgFileEntryId'] && thongTinDoanhNghiep['applicantXcgFileEntryId'] !== '0' ? "done" : "cancel"
                    }}
                  </v-icon>
                  Xem GCN
                </v-chip>
              ...
              <v-flex xs12 sm4 class="pt-2">
                <input v-if="!disabledForm" type="file" name="fileDieuKien" id="fileDieuKien">
              </v-flex>
            </v-layout>
          </v-flex>
          <v-flex xs12 sm4>
          ...
          <v-flex xs12 sm3>
            ...
            item-text="itemName"
            item-value="itemCode"
            v-model="thongTinDoanhNghiep['applicantBusinessType']"
            outline
            :class="{'track-change': isChange('applicantBusinessType')}"
            ></v-autocomplete>
          </v-flex>
...
export default {
  ...
  created () {
    ...
    vm.$nextTick(function () {
      ...
      console.log('applicantMetadata+++++++++', vm.thongTinDoanhNghiep)
      if (vm.thongTinDoanhNghiep['applicantMetadata']) {
        vm.dataNguoiDaiDienItems = JSON.parse(vm.thongTinDoanhNghiep['applicantMetadata']) || []
      }
      if (!vm.id || vm.id === '0') {
  ...
  watch: {
    ...
    //   this.thongTinDoanhNghiep['applicantMetadata'] = JSON.stringify(val)
    ...
    'thongTinDoanhNghiep.applicantMetadata' (val) {
      let vm = this
      console.log('watch val=====222======', val)
      if (val) {
        vm.dataNguoiDaiDienItems = JSON.parse(val) || []
      }
    }
  },
  methods: {
    isChange (key) {
      ...
      if (vm.thongTinDoanhNghiep[key] !== vm.thongTinChungPrev[key]) {
        return true
      }
      return false
    },
    ...
    xemGCNDangKy () {
      let vm = this
      if (!vm.thongTinDoanhNghiep['applicantIdentityFileEntryId'] || vm.thongTinDoanhNghiep['applicantIdentityFileEntryId'] === '0') {
        console.log('ko có applicantIdentityFileEntryId', vm.thongTinDoanhNghiep['applicantIdentityFileEntryId'])
        return
      }
      vm.$store.dispatch('getFileGCN', {
        fileEntryId: vm.thongTinDoanhNghiep['applicantIdentityFileEntryId'] || 0
      }).then(result => {
        console.log('result============++++++++', result)
        window.open(result, '_blank')
      }).catch(xhr => {
        toastr.error('Không tìm thấy dữ liệu, vui lòng thử lại!')
      })
    },
    xemGCNĐuieuKien () {
      let vm = this
      if (!vm.thongTinDoanhNghiep['applicantXcgFileEntryId'] || vm.thongTinDoanhNghiep['applicantXcgFileEntryId'] === '0') {
        console.log('ko có applicantXcgFileEntryId', vm.thongTinDoanhNghiep['applicantXcgFileEntryId'])
        return
      }
      vm.$store.dispatch('getFileGCN', {
        fileEntryId: vm.thongTinDoanhNghiep['applicantXcgFileEntryId'] || 0
      }).then(result => {
        console.log('result============++++++++', result)
        window.open(result, '_blank')
      }).catch(xhr => {
