<template>
  <div style="width: 100%; position: relative; border-right: 1px solid #ccc; height: 100%;">
    ...
    <v-layout row wrap>
      <v-flex xs12 sm6>
        <v-btn @click="capNhatThongTin()" :disabled="disabledStatusBtnUpdate" class="px-0 mx-0 py-0 mt-0 mb-1" color="primary" style="width: 100%;">Cập nhật</v-btn>
      </v-flex>
    ...
    <v-list two-line v-else class="list-history" style="height: 500px; overflow: auto;">
      <v-list-tile
...
export default {
  ...
  computed: {
    disabledStatus () {
      if ((!this.thongTinChung['applicantStatus'] || this.thongTinChung['applicantStatus'] === '0' || this.thongTinChung['applicantStatus'] === 3 || this.thongTinChung['applicantStatus'] === '3') && (this.thongTinChung['applicantName'] && this.thongTinChung['applicantAddress'] && this.thongTinChung['applicantissueplace'] && this.thongTinChung['applicantissuedate'])) {
        return false
      } else {
        return true
      }
    },
    disabledStatusBtnUpdate () {
      if (this.thongTinChung['applicantStatus'] === '2' || this.thongTinChung['applicantStatus'] === 2) {
        return false
      } else {
        return true
      }
  ...
  watch: {
    '$route': function (newRoute, oldRoute) {
      ...
      this.loaddDSNhaXuong(vm.thongTinChung['id'] || 0)
      let path = vm.$router.history.current.path
      if (path && path.indexOf('thong-tin-xuong') !== -1) {
        vm.activeIndex = 'ThongTinXuong'
      } else {
  ...
  methods: {
    eventLoadDsXuong (data) {
      let vm = this
      vm.loaddDSNhaXuong(data)
    },
    countDSNhaXuong () {
      let vm = this
    ...
    loaddDSNhaXuong (applicantprofileid) {
      let vm = this
      return new Promise((resolve, reject) => {
        let tmp = 0
        if (applicantprofileid) {
          tmp = applicantprofileid
        } else {
          tmp = vm.thongTinChung['id'] || vm.thongTinChung['applicantCode']
        }
        if (!tmp) {
          vm.listXuongItems = []
          resolve([])
        ...
        vm.$store.dispatch('getDsNhaXuong', {
          applicantprofileid: tmp,
          mtCore: 1
        }).then(result => {
          vm.listXuongItems = result['data']
          vm.countNhaXuong = result['total']
          resolve(vm.listXuongItems)
          console.log('countNhaXuong++++++++++++', vm.countNhaXuong)
        }).catch(xhr => {
          vm.listXuongItems = []
          vm.countNhaXuong = 0
          reject(xhr)
    ...
    async saveThongTinChung (state) {
      ...
      return new Promise((resolve, reject) => {
        ...
        vm.loaddDSNhaXuong().then(resultTTXuong => {
          if (state === 1 || state === '1') {
