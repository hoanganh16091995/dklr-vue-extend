    ...
    <div class="ml-0">
      <div class="row-header">
        ...
        <div class="layout row wrap header_tools row-blue">
          ...
          <div class="flex xs6 text-right pr-3" style="margin-left: auto;">
            <v-btn class=" my-0 py-0 btn-border-left" :disabled="disabledStatus" color="primary" v-on:click.native="saveThongTinChung(0)">
            ...
            <v-btn class=" my-0 py-0 btn-border-left" color="primary" v-on:click.native="quayLai()">
      ...
      <v-layout row wrap class="pt-2 ml-0">
        ...
        <v-flex xs12 sm12 class="py-2">
          <thong-tin-doanh-nghiep-form ref="thongTinDoanhNghiep" class="px-2" :thongTinDoanhNghiep="thongTinChung" :disabledForm="false" :style="{'pointer-events': disabledStatus ? 'none': ''}" :thongTinChungPrev="thongTinChungPrev"></thong-tin-doanh-nghiep-form>
        </v-flex>
        <v-flex xs12 sm12 class="text-xs-right">
          <v-btn color="primary" :disabled="disabledStatus" @click="saveThongTinChung(0)"><v-icon>save</v-icon>&nbsp; Lưu</v-btn>
...
export default {
  ...
  methods: {
    loadThongTinChung () {
      ...
      if (!vm.id || vm.id === '0') {
        vm.$store.dispatch('loadInitResource').then(function (result) {
          ...
          vm.$store.dispatch('getThongTinChung', {
            ...
            if (resultApplicant && resultApplicant['id']) {
              // delete resultApplicant['id']
              resultApplicant['applicantStatus'] = 0
              vm.$store.commit('setThongTinChung', resultApplicant)
              console.log('resultApplicant++++++++++', resultApplicant)
              if (vm.$refs.thongTinDoanhNghiep && result['applicantMetadata']) {
                vm.$refs.thongTinDoanhNghiep['dataNguoiDaiDienItems'] = JSON.parse(resultApplicant['applicantMetadata']) || []
              }
            }
          }).catch(xhr => {
          })
      ...
      vm.$store.dispatch('getThongTinChung', {
        applicantCode: vm.id || 0
      }).then(result => {
        console.log('getThongTinChung+++++++', result)
        if (vm.$refs.thongTinDoanhNghiep && result['applicantMetadata']) {
          vm.$refs.thongTinDoanhNghiep['dataNguoiDaiDienItems'] = JSON.parse(result['applicantMetadata']) || []
        }
      }).catch(xhr => {
      })
    },
    ...
    updateEmailAddress () {
      let vm = this
      vm.$store.dispatch('updateEmailAddress', {
        emailAddress: vm.thongTinChung['applicantEmail'],
        applicantIdNo: vm.thongTinChung['applicantCode'],
        applicantName: vm.thongTinChung['applicantName']
      }).then(result => {
        console.log('result============', result)
      }).catch(xhr => {
      })
    ...
    async saveThongTinChung (state) {
      ...
      if (vm.$refs.thongTinDoanhNghiep) {
        console.log('validate+++++++++++', this.$refs.thongTinDoanhNghiep.validFormDN)
        if (!this.$refs.thongTinDoanhNghiep.validFormDN) {
          toastr.error('Vui lòng kiểm tra lại thông tin bắt buộc!')
          return
        }
      }
      if ($('#fileGCN')[0].files && $('#fileGCN')[0].files['length']) {
        await this.$refs.thongTinDoanhNghiep.xacNhanTaiFileGCNDangKy()
      }

      if ($('#fileDieuKien')[0].files && $('#fileDieuKien')[0].files['length']) {
        await this.$refs.thongTinDoanhNghiep.xacNhanTaiFileĐuieuKien()
      }
      vm.thongTinChung['mtCore'] = 1
      return new Promise((resolve, reject) => {
        if (vm.$refs.thongTinDoanhNghiep) {
          let dataNguoiDaiDienItems = vm.$refs.thongTinDoanhNghiep['dataNguoiDaiDienItems'] || []
          this.thongTinChung['applicantMetadata'] = JSON.stringify(dataNguoiDaiDienItems)
        }
        ...
        vm.$store.dispatch('saveThongTin', query).then(result => {
          ...
          if (!vm.id || vm.id === '0') {
            ...
            vm.$router.push({
              path: '/thong-tin-chung/' + result['data']['vr_ApplicantProfile']['applicantCode']
            })
          } else {
            toastr.success('Lưu thành công!')
          }
          vm.updateEmailAddress()
        }).catch(xhr => {
